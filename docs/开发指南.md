...
> 本文档覆盖项目的目标、架构、开发命令与关键注意事项，截至 2026-08-16 的最新状态。
...
- **手机端特色**：Shizuku 特权插件 + 系统能力插件 + **原生控制台**（状态 / 权限 / 日志 / 数据管理）。
...
4. 锁横屏（sensorLandscape）；品牌只与 DeepSeek Harness 相关。
...
| 项 | 状态 |
|---|---|
| 安装包 | `DeepSeekHarness.apk`（约 75MB，已签名，见 GitHub Release） |
| 包名 | `com.deepseek.harness` |
...
| 首屏 | 原生控制台（状态 / 系统信息 / 权限 / 日志 / 操作 / 数据管理） |
...
├── res/                  图标 + 字符串资源
└── src/.../MainActivity.java   Android 原生壳（控制台/权限/数据管理/引擎控制）
...
│   │   └── 返回键：WebView 能后退就后退，否则回控制台（不退出 App）
...
- 改原生壳（控制台/权限/数据管理）：改 `android-app/src/.../MainActivity.java` → `bash build.sh` → 重装。
...
12. **⚠️ 原生 View 重复挂载崩溃**：`webView/statusView/progressBar` 是成员视图，`showEngineScreen()` 和 `showConsole()` 都要先 `detachView()` 再 addView，否则 "child already has a parent" 崩。新增视图时沿用 `detachView()`。
...
16. **dshroot 外部更新策略**：`FORCE_OVERWRITE_PREFIXES` 白名单强制覆盖官方插件（目前只有 dsh-tool-shizuku），其余已有文件不覆盖（保留运行时修改）。
